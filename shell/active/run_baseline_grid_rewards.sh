#!/bin/bash
# R27c: R25b exact rewards + ADD r_sg=1.5, r_sb=1.5
...
export STEMS_ALPHA_GRID=1.5           # NEW (was 0.0 in R25b)
export STEMS_ALPHA_BUILD=1.5          # NEW (was 0.0 in R25b)
...
export STEMS_ALPHA_EV_SMART=0.0       # OFF
...
echo "=== R27c: R25b rewards + r_sg=1.5 + r_sb=1.5 (40-epoch) ==="
...
nohup python scripts/train_multi_lag_stems.py \
    --cfg configs/active/baseline_grid_rewards.yaml \
    --hidden_dim 64 \
    --output_dim 256 \
    --num_gcn_layers 3 \
    --stems_critic \
    ...
    --temporal_pool mean \
    --partial_obs_norm \
    > /tmp/baseline_grid_rewards.log 2>&1 &

echo "PID: $!"
echo "Log: /tmp/baseline_grid_rewards.log"
