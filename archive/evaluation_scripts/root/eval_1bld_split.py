#!/usr/bin/env python3
"""
1-Building Split NEC-Sign Test -- Full Year Evaluation
======================================================
Loads the best 1bld_split checkpoint and runs 1 full year (8759 steps)
with deterministic actions (Gaussian mean).

Usage:
    python eval_1bld_split.py [--epoch 50]
"""
import os
import sys
import argparse
...
# =====================================================================
# ENV VARS -- match run but with FULL YEAR schema
# =====================================================================
def set_env():
    ...
    env_vars = {
        ...
        # --- NEC-sign reward (split version, STEMS_LAMBDA_EV=15.0) ---
...
# =====================================================================
CKPT_DIR = (
    f"{PROJECT}/runs/test_1bld_split/"
    "PPOLagMulti-{CityLearnSafety-V2G-v2}/"
    "seed-042-2026-03-19-21-20-27/torch_save"
)

OUT_DIR = f"{PROJECT}/runs/test_1bld_split"
...
def evaluate(epoch=50, seed=42):
    ...
    print(f"\n{'='*72}")
    print(f"  1-BUILDING SPLIT NEC-SIGN TEST -- FULL YEAR EVALUATION")
    ...
    # Price-action correlation (full year)
    prices_arr = np.array(price_all)
    if batt_actions.ndim > 1:
        batt_avg_per_step = batt_actions.mean(axis=1)
    else:
        batt_avg_per_step = batt_actions.ravel()
    valid = np.isfinite(prices_arr) & np.isfinite(batt_avg_per_step)
    if valid.sum() > 10:
        price_batt_corr = np.corrcoef(prices_arr[valid], batt_avg_per_step[valid])[0, 1]
    else:
        price_batt_corr = float('nan')

    # =====================================================================
    # PRINT RESULTS
    # =====================================================================
    print(f"\n{'='*72}")
    print(f"  1-BUILDING SPLIT NEC-SIGN EVALUATION RESULTS (epoch-{epoch}, full year)")
    print(f"{'='*72}")
    ...
    print(f"\n  {'='*60}")
    print(f"  PRICE-ACTION CORRELATION")
    print(f"  {'='*60}")
    print(f"  Price vs Battery action (full year): {price_batt_corr:.4f}")
    ...
    fig.suptitle(
        f"1-Building Split NEC-Sign Test (epoch-{epoch}, full year)\n"
    ...
    batt_avg_week = batt_actions[:week_len].mean(axis=1) if batt_actions.ndim > 1 else batt_actions[:week_len].ravel()
    ...
    ax2.plot(t_range, batt_avg_week, color='steelblue', alpha=0.7, linewidth=0.8, label='Batt action')
    ...
    ax.set_title(f"Price vs Battery Action (Week 1)\ncorr={price_batt_corr:.3f}")

    plt.tight_layout()
    out_path = os.path.join(OUT_DIR, "eval_split.png")
