#!/usr/bin/env bash
# R25b report-stable clean ablation:
#   Disable Sauté and explicitly disable C1-dense as well.
# This removes:
#   1. the Sauté budget observation
#   2. the Sauté unsafe reward path
#   3. the dense EV Lagrangian channel (C1) itself
set -euo pipefail
...
echo ""
echo "============================================"
echo "  R25b Report-Stable No-Saute / No-C1 Clean"
echo "  Same PPO/env/reward recipe as stable run"
echo "  Sauté OFF, dense EV cost OFF, C1 disabled in MultiLag"
echo "  Cleanest ablation of the full Sauté + C1 branch"
echo "============================================"
echo ""

python scripts/train_multi_lag.py \
    --cfg configs/on-policy/r25b_report_stable_nosaute_no_c1_clean.yaml
