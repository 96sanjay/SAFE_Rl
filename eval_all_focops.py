#!/usr/bin/env python3
"""Evaluate FOCOPS checkpoints: Free vs PSF. 1 episode each."""
...
parser.add_argument("--max_steps", type=int, default=8760)
...
def load_actor(ckpt_path):
    ...
    actor = GaussianActor()
    actor.load_state_dict(data["pi"])
    actor.eval()
    nd = data["obs_normalizer"]
    normalizer = ObsNormalizer(nd["_mean"], nd["_std"], nd["_clip"])
...
CONFIGS = [
    ...
    {
        "name": "FOCOPS-v3 (shield C1C2, 20ep)",
...
print(f"All checkpoints found. Seed={args.seed}\n")
...
def rollout(env, actor, normalizer, seed, max_steps, label=""):
    ...
    for step in range(max_steps):
        ...
        if n_dep > 0:
            c1_dep += n_dep
            deficit = info.get("ev_avoidable_deficit_kwh", 0.0)
            c1_deficit += deficit
    ...
    r = {
        "label": label, "steps": step_count,
        "reward": total_reward, "cost": total_cost,
        "c1_dep": c1_dep, "c1_viol": c1_viol,
        "c1_%": 100.0*c1_viol/max(c1_dep,1), "c1_kwh": c1_deficit,
        "c2_viol": c2_viol, "c2_%": 100.0*c2_viol/max(step_count,1),
        "c3_viol": c3_viol, "c3_%": 100.0*c3_viol/max(step_count,1),
        "c4_viol": c4_viol, "c4_%": 100.0*c4_viol/max(step_count,1),
        "time_s": elapsed,
    }
    if psf_times:
        r["psf_int"] = psf_interv
        r["psf_int_%"] = 100.0*psf_interv/max(step_count,1)
        r["psf_infeas"] = psf_infeas
        r["psf_ms"] = np.mean(psf_times)
        r["psf_delta"] = np.mean(psf_deltas)
    return r
...
for cfg in CONFIGS:
    ...
    for mode, builder, suffix in [("Free", build_env_free, " | Free"), ("PSF", build_env_psf, " | +PSF")]:
        ...
        print(f"{'='*60}")
        env = builder()
        res = rollout(env, actor, normalizer, seed=args.seed, max_steps=args.max_steps, label=label)
        env.close()
...
# ── Comparison table ──────────────────────────────────────────
print(f"\n\n{'='*105}")
print(f"  COMPARISON TABLE  (seed={args.seed}, 1 episode = {args.max_steps} steps)")
print(f"{'='*105}")
hdr = f"  {'Config':<44} {'Reward':>10} {'C1%':>7} {'C2%':>7} {'C3%':>7} {'C4%':>7} {'C1kwh':>8} {'PSF%':>7} {'Time':>6}"
print(hdr)
print("  " + "-"*(len(hdr)-2))
for r in all_results:
    psf_s = f"{r.get('psf_int_%',0):.1f}" if "psf_int" in r else "  n/a"
    print(f"  {r['label']:<44} {r['reward']:>10.0f} {r['c1_%']:>6.1f}% {r['c2_%']:>6.1f}% {r['c3_%']:>6.1f}% {r['c4_%']:>6.1f}% {r['c1_kwh']:>7.2f} {psf_s:>7} {r['time_s']:>5.0f}s")
print(f"\n  PSF% = fraction of steps where PSF intervened")
print(f"  C1% = fraction of EV departures with avoidable deficit > 0.01 kWh")
print(f"  C2/C3/C4% = fraction of steps with violation\n")
