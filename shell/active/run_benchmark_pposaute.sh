#!/bin/bash
# =============================================================================
# R28 PPOSaute benchmark — state-augmentation baseline
# =============================================================================
# Matches run_r28_benchmarks.sh env vars verbatim (same 9-term reward,
# same aggregate cost weights, same schema, same flags).
# Only PPOSaute-specific differences live in configs/active/benchmark_pposaute.yaml.
#
# Usage:
#   ./run_r28_pposaute.sh           # full 100-epoch run
#   ./run_r28_pposaute.sh --smoke   # 10-epoch local smoke test
# =============================================================================
set -euo pipefail
...
# ---------------------------------------------------------------------------
# Shared env vars (identical to run_r28_benchmarks.sh)
# ---------------------------------------------------------------------------
...
# Launch
# ---------------------------------------------------------------------------
CFG="configs/active/benchmark_pposaute.yaml"

if [[ $SMOKE -eq 1 ]]; then
    echo "=== PPOSaute SMOKE TEST (10 epochs, local) ==="
    # Use a smoke override config written alongside the main one
    SMOKE_CFG="configs/active/benchmark_pposaute_smoke.yaml"
    python - <<PY
import yaml, pathlib
cfg = yaml.safe_load(open("${CFG}"))
cfg["train_cfgs"]["total_steps"] = 87590  # 10 epochs
cfg["logger_cfgs"]["log_dir"] = "./runs/benchmark_pposaute_smoke/5bld"
pathlib.Path("${SMOKE_CFG}").write_text(yaml.safe_dump(cfg, sort_keys=False))
PY
    CFG="${SMOKE_CFG}"
fi

echo "Launching with config: ${CFG}"
