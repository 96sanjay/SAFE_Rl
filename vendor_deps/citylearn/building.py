...
class Building(Environment):
    ...
    **kwargs : Any
    ...
    def observations(self, include_all: bool = None, normalize: bool = None, periodic_normalization: bool = None, check_limits: bool = None) -> Mapping[str, float]:
        ...
        if check_limits:
            for k in self.active_observations:
                value = observations.get(k, 0.0)  # MISSING-OBS-FALLBACK
    ...
    def estimate_observation_space_limits(self, include_all: bool = None, periodic_normalization: bool = None) -> Tuple[
        ...
        # --- BEGIN: Merge EV_*.csv columns into data for EV observations (schema compatibility) ---
        # CityLearn core loads Building_<n>.csv into `data`; EV obs live in EV_<n>.csv, so we merge them here.
        try:
            import os
            import pandas as pd
            ev_obs_keys = {'charger_state','required_soc_departure','estimated_soc_arrival','arrival_time','departure_time','soc'}
            obs_list = set(getattr(self, 'observations', []) or [])
            active_obs_list = set(getattr(self, 'active_observations', []) or [])
            if ev_obs_keys & (obs_list | active_obs_list):
                root_dir = getattr(self, 'root_directory', None)
            if root_dir is None:
                root_dir = getattr(getattr(self, 'environment', None), 'root_directory', None)
            if root_dir is None:
                # Fallback: read root_directory from schema path in env (cached)
                import os, json
                sp = os.environ.get('CITYLEARN_SCHEMA', None)
                if sp:
                    if not hasattr(self, '_schema_root_directory_cache'):
                        try:
                            self._schema_root_directory_cache = json.load(open(sp)).get('root_directory', None)
                        except Exception:
                            self._schema_root_directory_cache = None
                    root_dir = self._schema_root_directory_cache
            ...
            if root_dir is None:
                ...
                if root_dir is not None:
                    import re as _re
                    bn = None
                    m = _re.search(r"(\d+)$", str(getattr(self, 'name', '')))
                    if m: bn = m.group(1)
                    if bn is None:
                        bn = str(getattr(self, 'building_id', '1'))
                    ev_path = os.path.join(str(root_dir), f"EV_{bn}.csv")
                    # DEBUG (prints once): where are we looking for EV CSV?
                    if not hasattr(self, '_ev_debug_printed'):
                        self._ev_debug_printed = True
                        print('EV-DEBUG name=', getattr(self,'name',None), 'building_id=', getattr(self,'building_id',None), 'root_dir=', root_dir, 'bn=', bn, 'ev_path=', ev_path)

                    if os.path.exists(ev_path):
                        ev_df = pd.read_csv(ev_path)
                        for c in ev_df.columns:
                            data[c] = ev_df[c].tolist()
        except Exception:
            pass
        # --- END: Merge EV_*.csv columns into data for EV observations (schema compatibility) ---
        ...
        for key in observation_names:
            ...
            if key == 'net_electricity_consumption':
                ...
                low_limit[key] = 0
                high_limit[key] = max(data[data_key])
                ...
                # --- EV observation key aliasing (schema keys -> EV CSV keys) ---
                alias = {
                    'charger_state': 'electric_vehicle_charger_state',
                    ...
                    'arrival_time': 'electric_vehicle_estimated_arrival_time',
                    'departure_time': 'electric_vehicle_departure_time',
                }
                data_key = key
                if data_key not in data and key in alias and alias[key] in data:
                    data_key = alias[key]
                # ---------------------------------------------------------------
                # --- Lazy-load EV CSV data if schema requests EV observations ---
                # Needed because EV columns live in EV_<n>.csv, not Building_<n>.csv.
                if 'data_key' in locals() and data_key not in data:
                    try:
                        import os
                        import pandas as pd
                        _alias = {
                        ...
                        if key in _alias:
                            ...
                            if root_dir is not None:
                                import re as _re
                                bn = None
                                mm = _re.search(r"(\d+)$", str(getattr(self, 'name', '')))
                                if mm: bn = mm.group(1)
                                if bn is None:
                                    bn = str(getattr(self, 'building_id', '1'))
                                ev_path = os.path.join(str(root_dir), f"EV_{bn}.csv")
                                if os.path.exists(ev_path):
                                    ...
                                    if col in ev_df.columns:
                                        data[col] = ev_df[col].tolist()
                                        data_key = col
                    except Exception:
                        pass
                # --- End lazy EV load ---
                # --- EV obs hard-mapping + lazy load (schema -> EV CSV) ---
                try:
                    _ev_map = {
                        'charger_state': 'electric_vehicle_charger_state',
                        'required_soc_departure': 'electric_vehicle_required_soc_departure',
                    ...
                    if key in _ev_map:
                        data_key = _ev_map[key]
                        if data_key not in data:
                            import os, pandas as pd, re as _re
                            root_dir = getattr(self, 'root_directory', None)
                            if root_dir is None:
                                root_dir = getattr(getattr(self, 'environment', None), 'root_directory', None)
                            if root_dir is not None:
                                ...
                                if os.path.exists(ev_path):
                                    ev_df = pd.read_csv(ev_path)
                                    if data_key in ev_df.columns:
                                        data[data_key] = ev_df[data_key].tolist()
                except Exception:
                    pass
                # --- end EV obs hard-mapping ---
                # EV-LIMITS-ENSURE-SERIES
                # Ensure EV CSV-backed series exist in `data` during observation space limit estimation.
                if 'data_key' in locals() and data_key not in data and isinstance(data_key, str) and data_key.startswith('electric_vehicle_'):
                    try:
                        import os, json, pandas as pd, re as _re
                        root_dir = getattr(self, 'root_directory', None)
                        if root_dir is None:
                            # Fallback to schema root_directory
                            sp = os.environ.get('CITYLEARN_SCHEMA', None)
                            if sp:
                                try:
                                    root_dir = json.load(open(sp)).get('root_directory', None)
                                except Exception:
                                    root_dir = None
                        if root_dir is not None:
                            # Determine building number from name like "Building_1"
                            bn = None
                            mm = _re.search(r"(\d+)$", str(getattr(self, 'name', '')))
                            if mm:
                                bn = mm.group(1)
                        ...
                        pass
                # If still missing, fall back to a safe placeholder so env can build.
                if 'data_key' in locals() and data_key not in data:
                    try:
                        # Use length of any existing series in data
                        _any = next(iter(data.values()))
                        _L = len(_any) if hasattr(_any, '__len__') else 1
                        data[data_key] = [0.0]*_L
                    except Exception:
                        data[data_key] = [0.0]
                low_limit[key] = min(data[data_key])
                # EV-HIGHLIMIT-ALIAS
                # If schema key (e.g., required_soc_departure) isn't in data but mapped data_key exists, alias it.
                if key not in data:
                    try:
                        if 'data_key' in locals() and data_key in data:
                            data[key] = data[data_key]
                    except Exception:
                        pass
                # If still missing, create a safe placeholder so env can build.
                if key not in data:
                    try:
                        _any = next(iter(data.values()))
                        _L = len(_any) if hasattr(_any, '__len__') else 1
...
class DynamicsBuilding(Building):
    ...
    **kwargs : Any
    ...
    def apply_actions(self, **kwargs):
        ...
        # Accept both '<action>' and '<action>_action' keys from env._parse_actions.
        # This makes central-agent wiring robust and fixes ignored HVAC/EV actions.
        for _k in list(kwargs.keys()):
            if isinstance(_k, str) and _k.endswith('_action'):
                _base = _k[:-7]
                if _base not in kwargs:
                    kwargs[_base] = kwargs[_k]

        # ACTION-KWARGS-FILTER (prevents TypeError when extra action keys exist)
        import inspect
        _parent_apply = super().apply_actions
        try:
            _sig = inspect.signature(_parent_apply)
            _allowed = {p for p in _sig.parameters.keys() if p != 'self'}
            _parent_kwargs = {k: v for k, v in kwargs.items() if k in _allowed}
        except Exception:
            _parent_kwargs = kwargs
        _parent_apply(**_parent_kwargs)

        # Keep a debug-friendly mirror even if parent ignores some keys
        if 'cooling_device' in kwargs:
            self.cooling_device_action = kwargs['cooling_device']
        # Capture cooling action for dynamics
        # (DynamicsBuilding only; used by _update_dynamics_input)
        if 'cooling_device' in kwargs:
            self.cooling_device_action = float(kwargs['cooling_device'])
        else:
            self.cooling_device_action = 0.0
    ...
    def update_indoor_dry_bulb_temperature(self):
        """RC-DYNAMICS-UPDATE-INDOOR: Advance indoor temperature using configured Dynamics (e.g., RCDynamics)."""
        # If no dynamics configured, keep default behavior (do nothing)
        if getattr(self, 'dynamics', None) is None:
            return

        # Read current indoor temp from a writable backing field if available
        tin = getattr(self, '_indoor_dry_bulb_temperature', None)
        if tin is None:
            tin = getattr(self, '_indoor_temperature', None)
        if tin is None:
            # fallback to property getter if it exists
            tin = getattr(self, 'indoor_dry_bulb_temperature', 22.0)

        # Outdoor temp from current observation data if available
        tout = None
        try:
            data = getattr(self, 'sim_results', None)
            if data is not None and hasattr(data, 'outdoor_dry_bulb_temperature'):
                # some versions store as list/array; use current time step index
                tout_series = data.outdoor_dry_bulb_temperature
                t = int(getattr(self, 'time_step', 0))
                tout = float(tout_series[t]) if hasattr(tout_series, '__len__') else float(tout_series)
        except Exception:
            tout = None
        if tout is None:
            # last resort: try attribute directly
            tout = float(getattr(self, 'outdoor_dry_bulb_temperature', 10.0))

        # Cooling action (normalized) from last applied actions
        u_cool = float(getattr(self, 'cooling_device_action', 0.0))

        # Step the dynamics model. Prefer CityLearn Dynamics API signature.
        try:
            obs = {'outdoor_dry_bulb_temperature': tout, 'indoor_dry_bulb_temperature': float(tin)}
            tin_next = self.dynamics.step(obs, u_cool)
        except TypeError:
            # backward-compatible signature: step(tout, u)
            tin_next = self.dynamics.step(tout, u_cool)

        # Write back to a writable backing field (property may be read-only)
        if hasattr(self, '_indoor_dry_bulb_temperature'):
            self._indoor_dry_bulb_temperature = float(tin_next)
        elif hasattr(self, '_indoor_temperature'):
            self._indoor_temperature = float(tin_next)
        else:
            self._indoor_dry_bulb_temperature = float(tin_next)
    ...
    def _update_dynamics_input(self):
        """Advance indoor temperature using the configured dynamics model (CityLearn Dynamics API)."""

        # Guard: do nothing if no dynamics configured
        if getattr(self, 'dynamics', None) is None:
            return

        # Outdoor temperature at current time step
        try:
            tout = float(self.weather.outdoor_dry_bulb_temperature[self.time_step])
        except Exception:
            tout = float(self.weather.dry_bulb_temperature[self.time_step])

        # Current indoor temperature (best-effort)
        tin_curr = float(getattr(self, '_indoor_dry_bulb_temperature', getattr(self, '_indoor_temperature', 22.0)))

        # Cooling action (0..1) captured in apply_actions
        u_cool = float(getattr(self, 'cooling_device_action', 0.0))
        u_cool = max(0.0, min(1.0, u_cool))

        # Observations dict expected by Dynamics.step(observations, action)
        # DYNAMICS-OBS-FULL
        # Build observation dict with all features required by dynamics (e.g., LSTM needs 13 inputs)
        obs = {}
        
        # If dynamics specifies required inputs, populate them from Building data
        if hasattr(self.dynamics, 'input_observation_names'):
            required_names = self.dynamics.input_observation_names
            time_step = self.time_step
            
            for obs_name in required_names:
                val = None
                
                # Try 1: Direct attribute access on energy_simulation
                try:
                    if hasattr(self, 'energy_simulation'):
                        es_attr = getattr(self.energy_simulation, obs_name, None)
                        if es_attr is not None:
                            if hasattr(es_attr, '__getitem__'):
                                val = float(es_attr[time_step])
                            else:
                                val = float(es_attr)
                except:
                    pass
                
                # Try 2: Weather object
                if val is None:
                    try:
                        if hasattr(self, 'weather'):
                            weather_attr = getattr(self.weather, obs_name, None)
                            if weather_attr is not None:
                                if hasattr(weather_attr, '__getitem__'):
                                    val = float(weather_attr[time_step])
                                else:
                                    val = float(weather_attr)
                    except:
                        pass
                
                # Try 3: Direct Building attribute
                if val is None:
                    try:
                        if hasattr(self, obs_name):
                            building_attr = getattr(self, obs_name, None)
                            if building_attr is not None:
                                if hasattr(building_attr, '__getitem__'):
                                    val = float(building_attr[time_step])
                                else:
                                    val = float(building_attr)
                    except:
                        pass
                
                # Store value (0.0 if not found)
                obs[obs_name] = val if val is not None else 0.0
        
        else:
            # For simple dynamics (like RC), just pass essential features
            obs['outdoor_dry_bulb_temperature'] = float(self.weather.outdoor_dry_bulb_temperature[self.time_step])
            obs['indoor_dry_bulb_temperature'] = float(self.indoor_dry_bulb_temperature[self.time_step])
        
        

        tin_next = self.dynamics.step(obs, u_cool)

        # Store into a writable backing field (property may be read-only)
        if hasattr(self, '_indoor_dry_bulb_temperature'):
            self._indoor_dry_bulb_temperature = float(tin_next)
        elif hasattr(self, '_indoor_temperature'):
            self._indoor_temperature = float(tin_next)
...
class LSTMDynamicsBuilding(DynamicsBuilding):
    ...
    **kwargs : Any
    ...
    def _update_dynamics_input(self):
        ...
        # LSTM-SETPOINT-FALLBACK-V2
        # SimBuild LSTM expects 'indoor_dry_bulb_temperature_set_point' in observations.
        # If missing, fall back to energy_simulation set point (safe; does not affect CS6/EV).
        if isinstance(observations, dict) and 'indoor_dry_bulb_temperature_set_point' not in observations:
            try:
                observations['indoor_dry_bulb_temperature_set_point'] = self.energy_simulation.indoor_dry_bulb_temperature_set_point
            except Exception:
                observations['indoor_dry_bulb_temperature_set_point'] = getattr(self.energy_simulation, 'indoor_dry_bulb_temperature_cooling_set_point', 0.0)
        ...
        self.dynamics._model_input = [            # LSTM-SETPOINT-FALLBACK
...
# ============================================================
# GUARDED LSTM TEMPERATURE CONTROL PATCH (safe-by-default)
# Activates only when a building uses LSTMDynamics.
# Does NOT affect datasets without LSTMDynamics (e.g., 2022 EV schema).
# ============================================================

def _cl_get_active_dynamics(building):
    dyn = getattr(building, "dynamics", None)
    if dyn is None:
        return None
    if isinstance(dyn, dict):
        if "cooling" in dyn:
            return dyn["cooling"]
        try:
            return next(iter(dyn.values()))
        except Exception:
            return None
    return dyn

def _cl_is_lstm_dynamics(dyn):
    return dyn is not None and ("LSTMDynamics" in type(dyn).__name__)

# Patch Building._get_observations_data: couple cooling_demand + expose predicted Tin
try:
    _cl_orig_get_obs = Building._get_observations_data
except Exception:
    _cl_orig_get_obs = None

def _cl_patched_get_observations_data(self):
    data = _cl_orig_get_obs(self)

    dyn = _cl_get_active_dynamics(self)
    if _cl_is_lstm_dynamics(dyn):
        # FIXED: Read cooling_demand directly from energy_simulation (set by update_cooling_demand)
        # Original patch was: data["cooling_demand"] = u_agent * cooling_demand_without_control
        # This was wrong because it ignored the value set by update_cooling_demand()
        if hasattr(self, "energy_simulation") and hasattr(self.energy_simulation, "cooling_demand"):
            try:
                t = getattr(self, "time_step", 0)
                data["cooling_demand"] = float(self.energy_simulation.cooling_demand[t])
            except Exception:
                pass

        # Observed Tin should be predicted Tin when available
        if hasattr(self, "_last_predicted_tin"):
            try:
                data["indoor_dry_bulb_temperature"] = float(getattr(self, "_last_predicted_tin"))
            except Exception:
                pass

    return data

if _cl_orig_get_obs is not None:
    Building._get_observations_data = _cl_patched_get_observations_data

# Patch DynamicsBuilding.update_indoor_dry_bulb_temperature: use same obs dict + freeze u_model
try:
    _cl_orig_update = DynamicsBuilding.update_indoor_dry_bulb_temperature
except Exception:
    _cl_orig_update = None

def _cl_patched_update_indoor_dry_bulb_temperature(self):
    dyn = _cl_get_active_dynamics(self)
    if dyn is None:
        return

    if _cl_is_lstm_dynamics(dyn):
        obs = self._get_observations_data()
        try:
            tin_next = float(dyn.step(obs, 0.0))  # freeze u_model to avoid conflicting control channel
        except TypeError:
            tin_next = float(dyn.step(obs))
        self._last_predicted_tin = tin_next
        return

    if _cl_orig_update is not None:
        return _cl_orig_update(self)

if _cl_orig_update is not None:
    DynamicsBuilding.update_indoor_dry_bulb_temperature = _cl_patched_update_indoor_dry_bulb_temperature

# ============================================================
# END GUARDED LSTM TEMPERATURE CONTROL PATCH
# ============================================================
