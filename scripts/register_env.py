...
def _thunk_v3():
    base = make_base_env(central_agent=True)
    env = CityLearnSafetyEnvV3(base, soc_min=0.1, soc_max=0.9)
    max_steps = getattr(base, "time_steps", None)
    if not isinstance(max_steps, int) or max_steps <= 0:
        max_steps = 35040
    return TimeLimit(env, max_episode_steps=max_steps)
...
if __name__ == "__main__":
    ...
    assert isinstance(env3.unwrapped, CityLearnSafetyEnvV3), f"SimpleV3-v0 failed, got {type(env3.unwrapped)}"
