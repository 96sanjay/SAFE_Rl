#!/bin/bash
# Ablation: GradS vs Softmax — Softmax arm
# R25b base with r_ev=1.0 (reduced from 5.0 so Lambda_0 stays active)
# Compare with run_ablation_grads.sh (identical except gradient method)
...
echo "=== Ablation: Softmax (multi-Lagrangian) ==="
echo "  Gradient method: Softmax (tau=1.0, all constraints contribute)"
...
nohup python scripts/train_multi_lag.py \
    --cfg configs/on-policy/r25b_softmax_ablation.yaml \
    > /tmp/ablation_softmax.log 2>&1 &

echo "PID: $!"
echo "Log: /tmp/ablation_softmax.log"
