...
class CityLearnEnv(Environment, Env):
    ...
    **kwargs : dict
    ...
    def _load_building(self, index: int, building_name: str, schema: dict, episode_tracker: EpisodeTracker, pv_sizing_data: pd.DataFrame, battery_sizing_data: pd.DataFrame, **kwargs) -> Building:
        ...
        # NORMALIZE-ENERGY-SIM-COLUMNS
        # Some datasets (e.g., SimBuild) use capitalized or space-separated column names (e.g., 'Month', 'Day Type').
        # EnergySimulation expects snake_case names (e.g., 'month', 'day_type'). Normalize safely.
        try:
            import re as _re
            def _snake(name: str) -> str:
                name = str(name).strip()
                # Replace non-alphanumerics with underscore, collapse repeats, lower-case
                name = _re.sub(r'[^0-9a-zA-Z]+', '_', name)
                name = _re.sub(r'_+', '_', name).strip('_')
                return name.lower()
            energy_simulation = energy_simulation.rename(columns={c: _snake(c) for c in energy_simulation.columns})
        except Exception:
            pass

        # FILTER-ENERGY-SIM-COLUMNS
        # Some datasets include extra columns (e.g., indoor_temperature_c) or non-CityLearn naming.
        # 1) Rename common aliases to CityLearn expected names.
        # 2) Filter dataframe to only the parameters accepted by EnergySimulation.__init__.
        try:
            from inspect import signature as _sig
            _allowed = set(_sig(EnergySimulation.__init__).parameters.keys()) - {"self"}
            _alias = {
                "indoor_temperature_c": "indoor_dry_bulb_temperature",
                "indoor_temperature": "indoor_dry_bulb_temperature",
            }
            _ren = {}
            for _k, _v in _alias.items():
                if _k in energy_simulation.columns and _v not in energy_simulation.columns:
                    _ren[_k] = _v
            if _ren:
                energy_simulation = energy_simulation.rename(columns=_ren)
            # Keep only columns EnergySimulation accepts
            energy_simulation = energy_simulation[[c for c in energy_simulation.columns if c in _allowed]]
        except Exception:
            pass

        # ENSURE-ENERGY-SIM-REQUIRED
        # Ensure required EnergySimulation columns exist.
        # Backward compatible: if columns already exist (e.g., CS6), nothing changes.
        try:
            import numpy as _np
            from inspect import signature as _sig
            _params = _sig(EnergySimulation.__init__).parameters
            _required = [k for k,v in _params.items()
                         if k not in ("self","seconds_per_time_step","noise_std") and v.default is v.empty]
            _n = len(energy_simulation)

            # Heuristic alias candidates (snake_case). We already snake-cased earlier.
            _alias_candidates = {
                "non_shiftable_load": ["non_shiftable_load", "electric_load", "electricity_load", "electricity_demand", "equipment_load", "appliance_load", "plug_load", "total_electric_load", "net_electric_load"],
                "dhw_demand": ["dhw_demand", "hot_water_demand", "domestic_hot_water_demand", "dhw_load", "hot_water_load"],
                "cooling_demand": ["cooling_demand", "cooling_load", "cooling_energy", "cooling", "hvac_cooling", "cooling_power"],
                "heating_demand": ["heating_demand", "heating_load", "heating_energy", "heating", "hvac_heating", "heating_power"],
                "solar_generation": ["solar_generation", "pv_generation", "pv_power", "solar_pv_generation", "solar_pv", "photovoltaic_generation"],
            }

            def _find_col(target):
                # exact or candidate name present?
                for c in _alias_candidates.get(target, []):
                    if c in energy_simulation.columns:
                        return c
                # substring fallback
                lc = [str(c).lower() for c in energy_simulation.columns]
                for i,c in enumerate(lc):
                    if target.split("_")[0] in c:  # e.g., 'cooling' in column name
                        return energy_simulation.columns[i]
                return None

            for _req in _required:
                if _req in energy_simulation.columns:
                    continue
                _src = _find_col(_req)
                if _src is not None and _src != _req:
                    energy_simulation[_req] = energy_simulation[_src]
                else:
                    # last-resort: fill with zeros to satisfy required args
                    energy_simulation[_req] = _np.zeros(_n, dtype=float)
        ...
        # Normalize weather column names (handles e.g., 'Outdoor Drybulb Temperature (C)') and filter to Weather.__init__ args.
        try:
            import re as _re
            import numpy as _np
            from inspect import signature as _sig
            ...
            weather = weather.rename(columns={c: _snake(c) for c in weather.columns})

            # 2) rename common aliases to CityLearn expected names
            _alias = {
                "outdoor_drybulb_temperature_c": "outdoor_dry_bulb_temperature",
                "outdoor_dry_bulb_temperature_c": "outdoor_dry_bulb_temperature",
                "outdoor_temperature_c": "outdoor_dry_bulb_temperature",
                "outdoor_relative_humidity_percent": "outdoor_relative_humidity",
                "outdoor_relative_humidity": "outdoor_relative_humidity",
                "diffuse_solar_radiation_w_m2": "diffuse_solar_irradiance",
                "diffuse_solar_irradiance_w_m2": "diffuse_solar_irradiance",
                "direct_solar_radiation_w_m2": "direct_solar_irradiance",
                "direct_solar_irradiance_w_m2": "direct_solar_irradiance",
            }
            _ren = {}
            for _k,_v in _alias.items():
                if _k in weather.columns and _v not in weather.columns:
                    _ren[_k] = _v
            if _ren:
                weather = weather.rename(columns=_ren)

            # 3) ensure required args exist (fill zeros if missing)
            _params = _sig(Weather.__init__).parameters
            _required = [k for k,v in _params.items() if k not in ("self","noise_std") and v.default is v.empty]
            _n = len(weather)
            for _req in _required:
                if _req not in weather.columns:
                    weather[_req] = _np.zeros(_n, dtype=float)

            # 4) filter to only allowed args
            _allowed = set(_params.keys()) - {"self"}
            weather = weather[[c for c in weather.columns if c in _allowed]]
        ...
        if building_schema.get('carbon_intensity', None) is not None:
            ...
            # NORMALIZE-CARBON-COLUMNS
            # Normalize carbon intensity columns (handles e.g., 'kg_CO2/kWh') and filter to CarbonIntensity.__init__ args.
            try:
                import re as _re
                import numpy as _np
                from inspect import signature as _sig
                ...
                carbon_intensity = carbon_intensity.rename(columns={c: _snake(c) for c in carbon_intensity.columns})

                # Common alias mapping
                _alias = {
                    "kg_co2_kwh": "carbon_intensity",
                    "kg_co2_per_kwh": "carbon_intensity",
                    "co2_intensity": "carbon_intensity",
                    "carbon_intensity_kg_co2_kwh": "carbon_intensity",
                }
                _ren = {}
                for _k,_v in _alias.items():
                    if _k in carbon_intensity.columns and _v not in carbon_intensity.columns:
                        _ren[_k] = _v
                if _ren:
                    carbon_intensity = carbon_intensity.rename(columns=_ren)

                _params = _sig(CarbonIntensity.__init__).parameters
                _required = [k for k,v in _params.items() if k not in ("self","noise_std") and v.default is v.empty]
                _n = len(carbon_intensity)
                for _req in _required:
                    if _req not in carbon_intensity.columns:
                        # If carbon_intensity is required and not present, create it
                        carbon_intensity[_req] = _np.zeros(_n, dtype=float)

                _allowed = set(_params.keys()) - {"self"}
                carbon_intensity = carbon_intensity[[c for c in carbon_intensity.columns if c in _allowed]]
        ...
        if building_schema.get('pricing', None) is not None:
            ...
            # NORMALIZE-PRICING-COLUMNS
            # Normalize pricing columns (handles e.g., 'Electricity Pricing [$/kWh]') and filter to Pricing.__init__ args.
            try:
                import re as _re
                import numpy as _np
                from inspect import signature as _sig
                ...
                pricing = pricing.rename(columns={c: _snake(c) for c in pricing.columns})

                # Common alias mapping to CityLearn expected name
                _alias = {
                    "electricity_pricing_kwh": "electricity_pricing",
                    "electricity_pricing_kwh_": "electricity_pricing",
                    "electricity_pricing": "electricity_pricing",
                    "price": "electricity_pricing",
                    "pricing": "electricity_pricing",
                    "electricity_price": "electricity_pricing",
                    "electricity_price_kwh": "electricity_pricing",
                }
                _ren = {}
                for _k,_v in _alias.items():
                    if _k in pricing.columns and _v not in pricing.columns:
                        _ren[_k] = _v
                if _ren:
                    pricing = pricing.rename(columns=_ren)

                _params = _sig(Pricing.__init__).parameters
                _required = [k for k,v in _params.items() if k not in ("self","noise_std") and v.default is v.empty]
                _n = len(pricing)
                for _req in _required:
                    if _req not in pricing.columns:
                        pricing[_req] = _np.zeros(_n, dtype=float)

                _allowed = set(_params.keys()) - {"self"}
                pricing = pricing[[c for c in pricing.columns if c in _allowed]]
        ...
        simbuild_multi_dyn = False  # SIMBUILD-FORCE-LSTMDYNAMICSBUILDING-ROBUST
        # set dynamics
        if building_schema.get('dynamics', None) is not None:
            # SIMBUILD-DYNAMICS-COOLING-SUBBLOCK
            # SimBuild schema format can be: dynamics={'cooling':{type,attributes}, 'heating':{...}}
            # CityLearn expects: dynamics={'type':..., 'attributes':...}. Use cooling sub-block as default.
            dyn = building_schema.get('dynamics', {})
            if isinstance(dyn, dict) and 'type' not in dyn and isinstance(dyn.get('cooling'), dict) and 'type' in dyn['cooling']:
                simbuild_multi_dyn = True

                building_schema['dynamics'] = dyn['cooling']
                from citylearn.building import LSTMDynamicsBuilding
                building_constructor = LSTMDynamicsBuilding
            ...
            # Only file-based dynamics (e.g., LSTMDynamics) need a filepath
            ...
            pass
            # PATCH: keep dynamics object
            # building_kwargs['dynamics'] = None

        # --- PATCH DISABLED: was forcing DynamicsBuilding, breaking LSTMDynamicsBuilding ---
        # if building_kwargs.get('dynamics', None) is not None and not building_schema.get('_simbuild_multidynamics', False) and not simbuild_multi_dyn:
        #     from citylearn.building import DynamicsBuilding
        #     building_constructor = DynamicsBuilding
# set occupant
        ...
        # SIMBUILD-FORCE-LSTMDYNAMICSBUILDING-ROBUST
        if simbuild_multi_dyn:
            from citylearn.building import LSTMDynamicsBuilding
            building_constructor = LSTMDynamicsBuilding
        ...
        for device_name in device_metadata:
            if building_schema.get(device_name, None) is None:
                ...
                device_random_seed = int(str(device_random_seed * # DEFAULT-RANDOM-SEED-WHEN-NONE
((schema.get('random_seed') or 0) + 1))[:9])
    ...
    def process_metadata(self, schema, building_schema, chargers_list, washing_machines_list, index, energy_simulation: EnergySimulation, **kwargs):
        ...
        if len(chargers_list) > 0:
            for charger in chargers_list:  # If present, iterate each charger
                ...
                # EV-CHARGER-ACTIONS-WIRE
                # Turn on per-charger EV storage actions (schema keys: electric_vehicle_storage_<charger_id>)
                ev_key = f"electric_vehicle_storage_{charger_id}"
                if chargers_actions_metadata_helper.get(ev_key, False):
                    action_metadata[ev_key] = True
