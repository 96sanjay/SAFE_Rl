#!/usr/bin/env python3
"""
R23 PPO Comprehensive Evaluation
=================================
Thorough evaluation of the R23 PPOLagMulti checkpoint covering:
 ...
 11. Solar-EV charging analysis (R23-specific)

Usage:
    python scripts/eval_r23_comprehensive.py [--epoch EPOCH] [--seed SEED]
"""
import os
import sys
import json
...
# =====================================================================
# Eval env vars — match R23 training config but DISABLE training wrappers
# =====================================================================
EVAL_ENV_VARS = {
    ...
    # R23 reward weights (match training)
    ...
    "STEMS_ALPHA_EV_SOLAR": "3.0",        # R23: NEW
    "COST_W_C2": "0.0",
    "COST_W_C3": "5.0",
}

# R23 constraint limits (same as R21)
COST_LIMITS = {
...
def evaluate(ckpt_path, seed=42):
    print(f"\n{'='*70}")
    print(f"  R23 PPO — COMPREHENSIVE EVALUATION")
    print(f"  (Solar-Aligned EV Charging Reward)")
    ...
    # --- Solar capacity for analysis ---
    solar_capacity = 0.0
    for b in buildings:
        pv = getattr(b, 'pv', None)
        if pv is not None:
            solar_capacity += abs(float(getattr(pv, 'nominal_power', 0.0) or 0.0))
    if solar_capacity <= 0:
        solar_capacity = 20.0
    print(f"  Solar capacity: {solar_capacity:.1f} kW")
    ...
    # === R23: Solar-EV charging tracking ===
    ev_charge_during_solar = 0      # EV charging actions when solar > 0
    ev_charge_total = 0             # Total EV charging actions
    ev_charge_solar_weighted = 0.0  # Sum of charge_action * solar_norm
    ev_charge_nosolar_weighted = 0.0
    hourly_ev_charge = [0.0] * 24   # Mean EV charge action per hour
    hourly_ev_counts = [0] * 24
    hourly_solar = [0.0] * 24       # Mean solar per hour
    hourly_solar_counts = [0] * 24
    # For battery comparison
    batt_charge_during_solar = 0
    batt_charge_total = 0
    ...
    while not done:
        ...
        # === R23: Get solar generation for this step ===
        solar_this_step = 0.0
        for b in buildings:
            try:
                s = getattr(b, 'solar_generation', None)
                if s is not None and hasattr(s, '__len__') and len(s) > t_idx:
                    solar_this_step += abs(float(s[t_idx]))
            except Exception:
                pass
        is_solar = solar_this_step > 0.1  # > 0.1 kW threshold
        solar_norm = min(solar_this_step / solar_capacity, 1.0) if solar_capacity > 0 else 0.0

        hourly_solar[hour] += solar_this_step
        ...
        # === V2G action tracking + R23 solar-EV analysis ===
        for ei in ev_idx:
            if ei < len(action):
                a = action[ei]
                total_ev_steps += 1
                if a > 0.05:
                    ...
                    ev_charge_total += 1
                    if is_solar:
                        ev_charge_during_solar += 1
                        ev_charge_solar_weighted += a * solar_norm
                    else:
                        ev_charge_nosolar_weighted += a
                elif a < -0.05:
                    ev_discharge_steps += 1
                else:
                    ev_idle_steps += 1
                # Track hourly EV charge action (all actions, not just >0.05)
                hourly_ev_charge[hour] += max(0.0, a)
                hourly_ev_counts[hour] += 1

        # === Battery solar tracking (for comparison) ===
        for bi in batt_idx:
            if bi < len(action):
                a = action[bi]
                if a > 0.05:
    ...
    # R23: Solar-EV metrics
    ev_solar_pct = pct(ev_charge_during_solar, ev_charge_total)
    batt_solar_pct = pct(batt_charge_during_solar, batt_charge_total)

    # Compute hourly solar-EV correlation
    hourly_ev_mean = [hourly_ev_charge[h] / max(1, hourly_ev_counts[h]) for h in range(24)]
    hourly_solar_mean = [hourly_solar[h] / max(1, hourly_solar_counts[h]) for h in range(24)]
    # Pearson correlation between hourly EV charging and solar
    ev_arr = np.array(hourly_ev_mean)
    sol_arr = np.array(hourly_solar_mean)
    if np.std(ev_arr) > 0 and np.std(sol_arr) > 0:
        solar_ev_corr = float(np.corrcoef(ev_arr, sol_arr)[0, 1])
    else:
        solar_ev_corr = 0.0
    ...
    # =====================================================================
    # R23-SPECIFIC: SOLAR-EV CHARGING ANALYSIS
    # =====================================================================
    section("8b. SOLAR-EV CHARGING ANALYSIS (R23)")
    print(f"  EV charges during solar:    {ev_charge_during_solar}/{ev_charge_total} "
          f"({ev_solar_pct:.1f}%)")
    print(f"  Battery charges during solar:{batt_charge_during_solar}/{batt_charge_total} "
          f"({batt_solar_pct:.1f}%)")
    print(f"  Solar-EV hourly correlation: r = {solar_ev_corr:.3f}")
    print(f"  R21 baseline:  EV solar=28.6%, Batt solar=58.3%, r=-0.21")
    print(f"  Target:        EV solar>45%, r>0")

    subsection("Hourly EV Charging vs Solar")
    print(f"  {'Hour':<6} {'Mean EV Charge':>14} {'Mean Solar(kW)':>14} {'EV/Solar Ratio':>14}")
    print(f"  {'-'*50}")
    for h in range(24):
        ev_h = hourly_ev_mean[h]
        sol_h = hourly_solar_mean[h]
        ratio_h = ev_h / max(0.001, sol_h) if sol_h > 0.1 else 0.0
        bar_ev = '#' * int(ev_h * 30)
        print(f"  {h:>4}h {ev_h:>14.4f} {sol_h:>14.2f} {ratio_h:>14.4f}  {bar_ev}")
    ...
    total_solar = sum(hourly_solar[h] for h in range(24))  # approximate
    ...
    subsection("R23 Solar-EV Summary vs R21 Baseline")
    print(f"  {'Metric':<30} {'R21':>10} {'R23':>10} {'Target':>10}")
    print(f"  {'-'*62}")
    print(f"  {'EV solar charging %':<30} {'28.6%':>10} {f'{ev_solar_pct:.1f}%':>10} {'>45%':>10}")
    print(f"  {'Solar-EV correlation':<30} {'-0.21':>10} {f'{solar_ev_corr:.3f}':>10} {'>0':>10}")
    print(f"  {'Batt solar charging %':<30} {'58.3%':>10} {f'{batt_solar_pct:.1f}%':>10} {'>50%':>10}")
    print(f"  {'Grid import (kWh)':<30} {'48520':>10} {f'{total_import:.0f}':>10} {'<48520':>10}")
    ...
    results = {
        ...
        "solar_ev_analysis": {
            "ev_solar_charging_pct": ev_solar_pct,
            "batt_solar_charging_pct": batt_solar_pct,
            "solar_ev_hourly_correlation": solar_ev_corr,
            "ev_charge_during_solar": ev_charge_during_solar,
            "ev_charge_total": ev_charge_total,
            "hourly_ev_charge_mean": hourly_ev_mean,
            "hourly_solar_mean": hourly_solar_mean,
        },
    ...
    out_path = os.path.join(PROJECT, "runs", "r23_ppo_eval_results.json")
