#!/usr/bin/env python3
"""
R15 Ablation Study — Comprehensive Evaluation

Evaluates R14 (baseline) + R15a/b/c/d with:
  1. Per-constraint violation percentages with CORRECT denominators:
     - C0 (EV departure): violations / total_departures (NOT per-timestep)
     - C1 (EV dense): handled by Sauté — report total cost only
     - C2 (battery SoC): violations / total_timesteps
     - C3 (building power): violations / total_timesteps (per-building)
     - C4 (grid power): violations / total_timesteps
  2. CityLearn standard KPIs (from evaluate())
  3. Intelligence metrics (solar charging, price-aware V2G, peak shaving, etc.)
  4. Ablation delta tables (R14→R15a→R15b→R15c→R15d)
"""
import os
import sys
import json
...
# =========================================================================
# Environment profile (same for all R14/R15 runs — no spatial, no temporal)
# =========================================================================
R15_ENV_VARS = {
...
# =========================================================================
RUN_DIRS = {
    "R14":  "runs/r14_pid_v2g/5bld",
    "R15a": "runs/r15a_clamp/5bld",
    "R15b": "runs/r15b_v2g_context/5bld",
    "R15c": "runs/r15c_peak_shave/5bld",
    "R15d": "runs/r15d_pid_fix/5bld",
}


def find_checkpoint(run_dir: str, epoch: int = None) -> str:
    """Find latest checkpoint in a run directory."""
    pattern = os.path.join(PROJECT, run_dir, "PPOLagMulti-*", "seed-*", "torch_save")
    save_dirs = sorted(glob_mod.glob(pattern))
    if not save_dirs:
        return ""
    save_dir = save_dirs[-1]  # Latest seed
...
# =========================================================================
# Episode runner with detailed per-constraint tracking
# =========================================================================
def run_eval_episode(actor, obs_mean, obs_std, obs_clip, actor_obs_dim, label):
    """Run 1 deterministic episode with detailed EV departure tracking."""
    ...
    need_pad = actor_obs_dim > env_obs_dim  # Sauté budget dim
    ...
    # EV departure tracking (CORRECT denominator)
    # Use environment's own departure detection via info dict
    # (manual detection fails because EV is already disconnected when we check)
    total_departures = 0
    violated_departures = 0
    departure_deficits = []  # SoC deficit at each departure

    # Track EV SoC each step for departure detection
    # Key: (b_idx, ch_idx) -> {was_connected, last_soc, required_soc}
    ev_tracker = {}
    ...
    while not done:
        # Pad obs if needed (Sauté budget dim = 1.0 = full budget)
        if need_pad:
        ...
        # Pre-step: track EV SoC BEFORE step (so we have it at departure)
            ...
            for ch_idx, ch in enumerate(chargers):
                ...
                try:
                    ...
                    if current_connected:
                        # Read SoC and required SoC while EV is still connected
                        ra = np.asarray(getattr(sim, '_electric_vehicle_required_soc_departure'),
                                       dtype=float)
                        ...
                        # Not connected — check if this is a departure
                        prev = ev_tracker.get(key, {})
                        if prev.get('was_connected', False):
                            # Departure detected! Use SAVED SoC from last connected step
                            total_departures += 1
    ...
    # Per-building C3 violation rates
    per_building_c3_pct = []
    ...
    # EV departure violation rate (CORRECT: violations / departures)
    ...
    # Peak shaving correlation: does battery discharge when building load is high?
    ...
    results = {
        ...
        # === CONSTRAINT VIOLATIONS (correct denominators) ===
        # C0: EV departure — violations / total departures
        ...
        # C1: Sauté-handled (total cost only)
        "c1_total_cost": float(sum(c1_vals)),

        # C2: Battery SoC — violations / timesteps
        ...
        # C3: Building power — violations / timesteps (+ per-building)
        ...
        # C4: Grid power — violations / timesteps
...
def print_report(all_results: dict):
    ...
    p()
    p("=" * 120)
    p("  R15 ABLATION STUDY — COMPREHENSIVE EVALUATION")
    ...
    def table_row(metric, key, fmt=".1f", extract=None):
        ...
        for v in vals:
            if isinstance(v, float) and np.isnan(v):
                ...
                cols += f"{v:>14{fmt}}"
        # Delta from R14
        if len(vals) >= 2 and isinstance(vals[0], (int, float)) and not np.isnan(vals[0]):
            for i in range(1, len(vals)):
                if isinstance(vals[i], (int, float)) and not np.isnan(vals[i]) and abs(vals[0]) > 1e-6:
                    pct = 100 * (vals[i] - vals[0]) / abs(vals[0])
        p(cols)
    ...
    # === Section 2: Constraint Violations (CORRECT denominators) ===
    p()
    ...
    for b in range(n_bld):
        table_row(f"  Building {b} Violation (%)",
                  None, ".1f",
    ...
    # === Section 6: Ablation Deltas ===
    p()
    p("  === ABLATION DELTAS (each run vs previous) ===")
    p()
...
# =========================================================================
def main():
    OUT_DIR = os.path.join(PROJECT, "runs", "r15_ablation", "evaluation")
    os.makedirs(OUT_DIR, exist_ok=True)

    print("\n=== R15 Ablation Evaluation ===\n")
    ...
    for name, run_dir in RUN_DIRS.items():
        ...
        if not ckpt:
            ckpt = find_checkpoint(run_dir)  # Any epoch
        if ckpt:
            agents[name] = ckpt
            print(f"  {name}: {ckpt}")
        else:
    ...
    # Save
    report_path = os.path.join(OUT_DIR, "r15_ablation_report.txt")
    with open(report_path, "w") as f:
        f.write(report_text)
    print(f"\nReport: {report_path}")

    # Save JSON
    json_path = os.path.join(OUT_DIR, "r15_ablation_results.json")
    json_results = {}
