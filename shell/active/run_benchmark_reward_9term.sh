#!/bin/bash
# R28b: 9-term reward, feasible limits, balanced EV signal
# Changes from R28a:
#   - r_ev 0.5→2.0 (restored dense charge urgency)
#   - r_ev_slack_arb 0.0→1.0 (restored solar-aware arbitrage + peak V2G)
#   - r_v2g_ctx 1.0→1.5 (partial restore from R25b's 3.0)
#   - r_ev_smart 1.0→1.5 (slight increase)
#   - r_eco REMOVED (harmful), r_load_shift REMOVED (dead),
#     r_peak_shave REMOVED (dead), r_ev_guard REMOVED (redundant)
#   - C3 limit 5000→18000, C4 limit 8000→13000 (feasible)
#   - C2 limit 1000→3500 (R25b achieved 3542)
#   - Lambda cap 35→25, C3/C4 PID reduced
#   - 100 epochs
set -euo pipefail
...
# ══════════════════════════════════════════════
# 9 ACTIVE REWARD TERMS
# ══════════════════════════════════════════════

# --- EV charging (4 terms, layered) ---
export STEMS_LAMBDA_EV="2.0"              # dense charge urgency (was 0.5 in R28a, 5.0 in R25b)
export STEMS_ALPHA_EV_SMART="1.5"         # headroom-gated price signal (was 1.0)
export STEMS_EV_SLACK_ARB_SCALE="1.0"     # RESTORED — solar-aware arb + peak V2G (was 0.0 in R28a, 2.0 in R25b)
export STEMS_ALPHA_V2G_CONTEXT="1.5"      # surplus EV context (was 1.0 in R28a, 3.0 in R25b)
...
export STEMS_ALPHA_LOAD_SHIFT="0.0"       # dead — batteries too constrained
...
echo "=== R28b: 9-Term Reward, Feasible Limits ==="
echo "  Active rewards: r_ev=2.0, r_ev_smart=1.5, r_ev_slack_arb=1.0,"
echo "                  r_v2g_ctx=1.5, r_barrier=0.5, r_sg=0.5,"
echo "                  r_sb=0.3, r_ramp=0.3, r_ren=0.2"
echo "  Disabled: r_ev_guard, r_grid_mild, r_eco, r_peak_shave, r_load_shift"
...
nohup python scripts/train_multi_lag.py \
    --cfg configs/active/benchmark_reward_9term.yaml \
    > /tmp/benchmark_reward_9term.log 2>&1 &

echo "PID: $!"
echo "Log: /tmp/benchmark_reward_9term.log"
