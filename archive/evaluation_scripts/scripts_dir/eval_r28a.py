#!/usr/bin/env python3
"""
Evaluation script for R28a (epoch 60) — MLP actor, no STEMS encoder.
...
Usage:
    ...
    python scripts/eval_r28a.py [--checkpoint PATH] [--device cpu]
"""
...
DEFAULT_CKPT = (
    "runs/r28a_clean9/5bld/"
    "PPOLagMulti-{CityLearnSafety-V2G-v2}/"
    "seed-042-2026-04-15-05-24-54/"
    "torch_save/epoch-60.pt"
)
...
# ---------------------------------------------------------------------------
def set_env_vars():
    """Set env vars matching run_r28a.sh exactly."""
    ...
    env_vars = {
        ...
        # Reward weights (R28a final state with KPI terms)
        "STEMS_LAMBDA_EV": "0.5",
        "STEMS_ALPHA_EV_GUARD": "0.0",
        "STEMS_ALPHA_EV_SMART": "1.0",
        "STEMS_EV_SLACK_ARB_SCALE": "0.0",
        "STEMS_ALPHA_V2G_CONTEXT": "1.0",
        ...
        "STEMS_ALPHA_PEAK_SHAVE": "0.3",
        "STEMS_ALPHA_LOAD_SHIFT": "0.3",
        # Cost weights
        "COST_W_C2": "0.0",
        "COST_W_C3": "5.0",
    ...
    for k, v in env_vars.items():
        os.environ[k] = v
    print(f"[eval_r28a] Schema: {schema_path}")


# ---------------------------------------------------------------------------
class SimpleMLP(nn.Module):
    """Mirrors the omnisafe gaussian_learning actor mean network."""
...
# ---------------------------------------------------------------------------
def build_eval_env():
    """Build env with same wrapper chain as train_multi_lag.py (no temporal)."""
...
def evaluate(args):
    ...
    model = SimpleMLP(obs_dim, act_dim, hidden_sizes=[256, 256])
    pi_sd = ckpt["pi"]
    mean_sd = {}
    for k, v in pi_sd.items():
        if k.startswith("mean."):
            # Keys are like mean.0.weight → net.0.weight
            new_key = "net." + k[len("mean."):]
            mean_sd[new_key] = v
    missing, unexpected = model.load_state_dict(mean_sd, strict=False)
    print(f"  Loaded {len(mean_sd)} tensors (missing={len(missing)}, unexpected={len(unexpected)})")
    if missing:
    ...
    print("\n")
    print("=" * 76)
    print("  R28a EVALUATION RESULTS (epoch 60, 9-term reward)")
    print("=" * 76)
    ...
    print(f"\n  " + "-" * 72)
    print(f"  {'Constraint':<25s} | {'Total Cost':>12s} | {'Viol %':>8s} | "
    ...
    for key, label in zip(COST_KEYS, COST_LABELS):
        ...
        max_c = np.max(costs)
        print(f"  {label:<25s} | {total_c:>12.1f} | {viol_pct:>7.1f}% | "
    ...
    print("\n" + "=" * 76)
    print("  R28a evaluation complete.")
    print("=" * 76)


# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Evaluate R28a checkpoint (MLP actor)")
    parser.add_argument("--checkpoint", type=str, default=None,
                        help="Path to checkpoint (default: epoch-60)")
