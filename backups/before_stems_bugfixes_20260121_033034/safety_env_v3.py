...
class CityLearnSafetyEnvV3(gym.Env):
    ...
    def _compute_stems_reward(self, kpis: Dict[str, float]) -> Dict[str, float]:
        ...
        # Building-level: penalize per-building consumption relative to building capacity
        # (approximation: use average across district)
        building_consumption = abs(net_consumption) / 17.0  # 17 buildings
        ...
        ramp_delta = float(kpis.get("grid_ramp_delta", 0.0))
        ramp_ratio = ramp_delta / max(1e-6, self.P_building_max)
