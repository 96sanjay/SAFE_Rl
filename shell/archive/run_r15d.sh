#!/usr/bin/env bash
# R15d: R15c + PID C0 Ki=0.05 fix
# Goal: Complete R15 ablation — add lambda integral memory for C0
#
# Changes from R15c:
#   - PID C0 Ki=0.05 (was 0.0): gives C0 integral memory
#   - No new env vars — Ki change is yaml-only
#
# Inherited from R15c:
#   - Hard EV action clamp + anti-discharge penalty
#   - Context-aware V2G signal + peak shaving
#   - PID Lagrangian + Sauté MDP (d=25000, shaped)
set -euo pipefail
...
# --- R15c: Peak shaving ---
export STEMS_ALPHA_PEAK_SHAVE="2.0"

# (R15d: PID Ki change is yaml-only — pid_ki_0: 0.05)

echo ""
echo "============================================"
echo "  R15d: R15c + PID C0 Ki=0.05 Fix"
echo "  Changes from R15c:"
echo "    - PID C0 Ki=0.05 (was 0.0)"
echo "    - Gives C0 integral memory"
echo "  Inherited from R15c:"
echo "    - Hard EV clamp + guard"
echo "    - V2G context (alpha=3.0)"
echo "    - Peak shaving (alpha=2.0)"
echo "    - Sauté MDP (d=25000, shaped alpha=10)"
echo "============================================"
echo ""

python scripts/train_multi_lag.py \
    --cfg configs/on-policy/r15d_pid_fix.yaml
