...
class ActionProjectionSERL(gym.Wrapper):
    ...
    def _get_battery_socs(self) -> list[float]:
        ...
        socs = []
        for b_idx in range(len(self._city.buildings)):
            soc, _, _, _ = self._get_battery_state(b_idx)
            socs.append(soc)
        return socs

    def _get_battery_state(self, b_idx: int) -> tuple[float, float, float, float]:
        """Read battery state using the current env timestep convention.

        Returns `(soc, capacity_kwh, nominal_power_kw, round_trip_efficiency)`.
        The SoC sample is aligned with the same `time_step - 1` convention used
        by the env-side reward/KPI logic.
        """
        soc = 0.5
        cap = 6.4
        ...
        try:
            es = getattr(self._city.buildings[b_idx], 'electrical_storage', None)
            if es is None:
                return soc, cap, p_batt, rte

            cap = float(getattr(es, 'capacity', cap) or cap)
            p_batt = float(getattr(es, 'nominal_power', p_batt) or p_batt)
            rte = float(getattr(es, 'round_trip_efficiency', rte) or rte)
            ...
            if hasattr(soc_arr, '__len__') and len(soc_arr) > 0:
                t = int(getattr(self._city, 'time_step', 0))
                idx = max(0, t - 1)
                idx = min(idx, len(soc_arr) - 1)
                soc = float(soc_arr[idx])
            else:
                soc = float(soc_arr)
        except Exception:
            pass
    ...
    @staticmethod
    def _battery_soc_scales(
        p_batt: float,
        cap: float,
        rte: float,
    ) -> tuple[float, float]:
        """Return charge/discharge SoC deltas per unit action.

        Match the asymmetric battery dynamics used by the SP-RL projector:
        charge uses `sqrt(rte)`, discharge uses `1 / sqrt(rte)`.
        """
        import math

        sqrt_rte = math.sqrt(max(rte, 0.01))
        scale_ch = (p_batt * sqrt_rte) / max(cap, 1e-6)
        scale_dis = (p_batt / sqrt_rte) / max(cap, 1e-6)
        return scale_ch, scale_dis
    ...
    def _ev_power_from_action(self, b_idx: int, action: float) -> float:
        """Map signed EV action to signed building power contribution in kW."""
        a = float(action)
        if a >= 0.0:
            return a * self._ev_max_charge.get(b_idx, 0.0)
        return a * self._ev_max_discharge.get(b_idx, 0.0)

    def _ev_power_upper_lower_expr(self, b_idx: int, z_expr):
        """Return convex-safe upper/lower affine envelopes for EV power.

        Real EV power is piecewise linear in the signed action because charge
        and discharge rates can differ. Mixed-integer modeling would be exact
        but is inappropriate here. Instead use affine envelopes that satisfy:

            lower(z) <= true_power(z) <= upper(z)   for z in [-1, 1]

        This preserves safety when we use `upper` for import caps and `lower`
        for export caps.
        """
        p_ch = self._ev_max_charge.get(b_idx, 0.0)
        p_dis = self._ev_max_discharge.get(b_idx, 0.0)

        if p_ch >= p_dis:
            # Convex case: secant is a safe upper envelope, discharge branch is
            # a safe lower envelope.
            slope = 0.5 * (p_ch + p_dis)
            bias = 0.5 * (p_ch - p_dis)
            upper = slope * z_expr + bias
            lower = p_dis * z_expr
        else:
            # Concave case: charge branch is a safe upper envelope, secant is a
            # safe lower envelope.
            slope = 0.5 * (p_ch + p_dis)
            bias = 0.5 * (p_ch - p_dis)
            upper = p_ch * z_expr
            lower = slope * z_expr + bias
    ...
    def _qp_project(
        ...
        if all_feasible:
            ...
            for b_idx in range(self._n_buildings):
                ...
                if b_idx in self._building_ev_act:
                    net_b += self._ev_power_from_action(
                        b_idx,
                        clipped[self._building_ev_act[b_idx]],
                    )
        ...
        for b_idx in self._building_batt_act:
            act_idx = self._building_batt_act[b_idx]
            soc, cap, p_batt, rte = self._get_battery_state(b_idx)
            scale_ch, scale_dis = self._battery_soc_scales(p_batt, cap, rte)
        ...
        #
        # For EVs, use affine envelopes around the signed piecewise power map:
        # `net_upper` over-approximates true net power and `net_lower`
        # under-approximates it. Enforcing `net_upper <= P_bmax` and
        # `net_lower >= -P_bmax` is conservative but safety-preserving.
        net_upper_exprs = []
        for b_idx in range(self._n_buildings):
            ...
            if b_idx in self._building_batt_act:
                act_idx = self._building_batt_act[b_idx]
                batt_term = self._batt_powers.get(b_idx, 0) * z[act_idx]
                net_upper = net_upper + batt_term
                net_lower = net_lower + batt_term
            if b_idx in self._building_ev_act:
                act_idx = self._building_ev_act[b_idx]
                ev_upper, ev_lower = self._ev_power_upper_lower_expr(b_idx, z[act_idx])
                net_upper = net_upper + ev_upper
                net_lower = net_lower + ev_lower
            constraints.append(net_upper <= self._p_bmax)
            constraints.append(net_lower >= -self._p_bmax)
            net_upper_exprs.append(net_upper)

        # C4: Grid aggregate import bound
        if self._c4_enabled and net_upper_exprs:
            grid_total = sum(net_upper_exprs)
    ...
    def _compute_safe_bounds(
        ...
        for b_idx in range(self._n_buildings):
            ...
            if has_batt and batt_act_idx is not None and p_batt > 0:
                ...
                try:
                    _, cap, _, rte = self._get_battery_state(b_idx)
                except Exception:
                    cap = 6.4
                    rte = 0.9

                soc_per_unit_charge, soc_per_unit_discharge = self._battery_soc_scales(
                    p_batt,
                    cap,
                    rte,
                )
