#!/usr/bin/env bash
# R15c: R15b + peak-shaving reward
# Goal: Teach agent to use batteries for building peak reduction (C3)
#
# Changes from R15b:
#   - Peak-shaving reward (alpha=2.0): battery discharge during building peaks
#
# Inherited from R15b:
#   - Hard EV action clamp + anti-discharge penalty
#   - Context-aware V2G signal (alpha=3.0)
#   - Sauté MDP (d=25000, shaped alpha=10)
#   - PID Lagrangian + all V2G fixes
set -euo pipefail
...
# --- R15c: Peak-shaving reward ---
export STEMS_ALPHA_PEAK_SHAVE="2.0"

echo ""
echo "============================================"
echo "  R15c: R15b + Peak-Shaving Reward"
echo "  Changes from R15b:"
echo "    - Peak shaving reward (alpha=2.0)"
echo "    - Battery discharge during building peaks"
...
python scripts/train_multi_lag.py \
    --cfg configs/on-policy/r15c_peak_shave.yaml
