...
"""
No-Control Baseline - FIXED to use CityLearnSafetyEnv backbone

Changes from original:
1. Uses make_base_env() + CityLearnSafetyEnv wrapper (same as OmniSafe)
2. KPIs now come directly from info dict (consistent with RL)
3. Safety cost is computed by wrapper (same formula as RL)
"""
...
def main():
    print("=" * 60)
    print(" Running No-Control Baseline (UNIFIED BACKBONE)")
    print("=" * 60)
    ...
    # Output paths
    runs_dir = Path("runs/baselines/no_control")
    ...
    while not done:
        ...
        row = {
            ...
            # Grid/energy metrics (from wrapper)
            ...
            # Prices/carbon intensity (from wrapper)
    ...
    # Get CityLearn evaluation metrics at episode end
