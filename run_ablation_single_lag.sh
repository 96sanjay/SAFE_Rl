#!/usr/bin/env bash
# Ablation: Single-Lambda PPOLag vs Multi-Lambda PPOLagMulti+PID (R21)
# Usage: ./run_ablation_single_lag.sh [seed]
#
# Reward terms IDENTICAL to R21. Only difference:
#   - Single aggregate lambda (OmniSafe PPOLag) instead of per-constraint multi-lambda+PID
#   - No C0 curriculum (single lambda can't anneal per-constraint)
# Baseline = R21 (PPOLagMulti + PID, already trained).
set -euo pipefail
...
# --- NO action mask (same as R21) ---
export CITYLEARN_ACTION_MASK="0"

RUN_DIR="runs/ablation_single_lag_s${SEED}"

echo ""
echo "============================================"
echo "  ABLATION: Single-Lambda PPOLag (seed=$SEED)"
echo "  Reward: IDENTICAL to R21"
echo "  Lagrangian: Single lambda on aggregate cost"
echo "  NO PID, NO per-constraint lambdas"
echo "  Baseline: R21 (multi-lambda + PID)"
...
/home/christmas/miniconda3/envs/citylearn/bin/python scripts/train_omnisafe.py \
    --cfg configs/on-policy/ablation_single_lag.yaml \
