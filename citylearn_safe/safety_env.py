...
from citylearn_safe.extractors import ev_departure_cost_components_v3
...
class CityLearnSafetyEnv(gym.Env):
    ...
    def __init__(
        ...
        *,
        ...
        if self._missing_action_mode not in {"assume_full", "assume_zero", "error"}:
            print(
                f"[CityLearnSafetyEnv] Warning: invalid CITYLEARN_EV_MISSING_ACTION_MODE={self._missing_action_mode}, using assume_full"
            )
        ...
        print("[CityLearnSafetyEnv] Initialized (V3 action-based EV deficits)")
        print(f"[CityLearnSafetyEnv] EV action indices (from action_names): {self._ev_charger_action_indices}")
        print(f"[CityLearnSafetyEnv] Missing action mode: {self._missing_action_mode}")
    ...
    def _discover_ev_action_indices(self) -> List[int]:
        ...
        if not isinstance(names, list) or len(names) == 0:
            raise RuntimeError("[CityLearnSafetyEnv] Could not read action_names to discover EV indices.")
        ...
        if len(ev_idx) == 0:
            print("[CityLearnSafetyEnv] Note: No EV chargers found. EV costs disabled.")
            return []
        return ev_idx
    ...
    def _ensure_kpi_logger_initialized(self):
        ...
        print(f"[CityLearnSafetyEnv] KPI logger initialized in: {log_dir} (run_name={run_name})")
...
# -------------------------------------------------------------------------
    ...
    def step(self, action):
        ...
        # --- Per-building diagnostics (C3 per-building rate for thesis ablation) ---
        # Emits 1.0 if that building's |NEC| exceeded P_building_max at this step,
        # else 0.0. building_nec_by_building carries the absolute NEC for each
        # building. These are additive-only; the aggregate cost and violation
        # fields above are unchanged.
        try:
            if "violations_list" in locals() and "building_powers" in locals():
                info["building_power_violation_by_building"] = [
                    float(1.0 if v > 0.0 else 0.0) for v in violations_list
                ]
                info["building_nec_by_building"] = [
                    float(abs(p)) for p in building_powers
                ]
            else:
                info["building_power_violation_by_building"] = []
                info["building_nec_by_building"] = []
        ...
        try:
            ...
            print(f"[CityLearnSafetyEnv] Warning: Could not compute EV costs: {e}")
        ...
        # Unified EV service cost: combines sparse departure failure and dense
        # charging shortfall into one opt-in trainer-facing signal. Old logic
        # stays intact because legacy keys and total_cost below are unchanged.
        ev_service_departure_w = float(os.environ.get("CITYLEARN_EV_SERVICE_W_DEPARTURE", "1.0"))
        ev_service_dense_w = float(os.environ.get("CITYLEARN_EV_SERVICE_W_DENSE", "1.0"))
        ev_service_cost = (
            ev_service_departure_w * float(ev_cost_for_cmdp)
            + ev_service_dense_w * float(ev_dense_cost)
        )
        info["cost_ev_service"] = float(ev_service_cost)
        info["cost_ev_service_departure_component"] = float(ev_service_departure_w * float(ev_cost_for_cmdp))
        info["cost_ev_service_dense_component"] = float(ev_service_dense_w * float(ev_dense_cost))
        ...
        # R27a: Merge dense EV corridor cost into C0 (Lambda_0 gets dense gradient)
        # C1 stays unused. V2G is unaffected — dense cost skips EVs with surplus SoC.
        if os.environ.get("CITYLEARN_C0_DENSE_MERGE", "0") == "1":
            info["cost_ev_departure"] = float(ev_cost_for_cmdp) + float(ev_dense_cost)
            info["cost_ev_dense"] = 0.0
        ...
        for b in getattr(citylearn_env, "buildings", []):
            for ch in (getattr(b, "electric_vehicle_chargers", None) or []):
                try:
                    ...
                    deficit = max(0.0, _c0_floor * req_soc - ev_soc_now)
                    if deficit <= 1e-6:
                        continue
