#!/bin/bash
# R28c_sac: SAC-Lag version of R28c (10-term reward)
# Same rewards/limits as R28c PPO, runs on CPU
set -euo pipefail
...
# ══════════════════════════════════════════════
# 10 ACTIVE REWARD TERMS (same as R28c PPO)
# ══════════════════════════════════════════════
...
export STEMS_ALPHA_LOAD_SHIFT="0.5"
...
echo "=== R28c_sac: SAC-Lag, 10-Term Reward, CPU ==="
...
echo "  SAC: buffer=1M, auto_alpha, warmup=5ep, CPU"
echo ""

nohup python scripts/train_sac_lag.py \
    --cfg configs/active/benchmark_sac_loadshift.yaml \
    > /tmp/benchmark_sac_loadshift.log 2>&1 &

echo "PID: $!"
echo "Log: /tmp/benchmark_sac_loadshift.log"
