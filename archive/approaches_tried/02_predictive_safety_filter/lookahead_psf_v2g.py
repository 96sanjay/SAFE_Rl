...
LookaheadPSF V2G: Predictive Safety Filter with V2G support.
  ...
  - Recursive-feasibility C1 prefixes: each early EV action must preserve
    a feasible path to departure if future charging saturates at its max
...
class LookaheadPSFv2G(gym.Wrapper):
    ...
    Key differences from original LookaheadPSF:
      ...
      2b. Recursive-feasibility C1 prefixes: early discharge is blocked when
          it would make the departure target unreachable later in the horizon
    ...
    def __init__(self, env, horizon: int = 24,
        ...
        self.recursive_c1 = recursive_c1
        self.verbose = verbose
        solver_order = os.environ.get("PSF_SOLVER_ORDER", "OSQP,CLARABEL,SCS")
        self.solver_order = [s.strip().upper() for s in solver_order.split(",") if s.strip()]
        self.osqp_max_iter = int(os.environ.get("PSF_OSQP_MAX_ITER", "5000"))
        self.osqp_eps_abs = float(os.environ.get("PSF_OSQP_EPS_ABS", "1e-3"))
        self.osqp_eps_rel = float(os.environ.get("PSF_OSQP_EPS_REL", "1e-3"))
    ...
    def _compile_qp(self):
        ...
        if n_ev > 0:
            ...
            self._p_ev_prefix_min = cp.Parameter(
                (n_ev, H), value=np.zeros((n_ev, H)), nonneg=True)
        ...
        if n_ev > 0:
            for ei in range(n_ev):
                energy_terms = cp.multiply(self._v_a_ev[ei, :], self._p_ev_c1_coeff[ei, :])
                energy = cp.sum(energy_terms)
                constraints += [energy >= self._p_ev_deficit[ei] - self._v_slack_c1[ei]]
                if self.recursive_c1:
                    for k in range(H):
                        prefix_energy = cp.sum(energy_terms[:k + 1])
                        constraints += [
                            prefix_energy >= self._p_ev_prefix_min[ei, k] - self._v_slack_c1[ei]
                        ]
    ...
    def _update_params_and_solve(self, proposed_flat):
        ...
        if self._max_ev > 0:
            ...
            ev_prefix_min = np.zeros((self._max_ev, H))
            ...
            for ei, evc in enumerate(self._mapping["ev_chargers"]):
                ...
                if self.recursive_c1:
                    future_max = np.cumsum(ev_c1_coeff[ei, ::-1])[::-1]
                    for k in range(H):
                        remaining_after_k = future_max[k + 1] if (k + 1) < H else 0.0
                        ev_prefix_min[ei, k] = max(0.0, ev_deficit[ei] - remaining_after_k)

            self._p_ev_gain.value = ev_gain
            self._p_ev_c1_coeff.value = ev_c1_coeff
            self._p_ev_deficit.value = ev_deficit
            self._p_ev_prefix_min.value = ev_prefix_min
            self._p_proposed_ev.value = proposed_ev

        self._p_grid_limit.value = float(self.p_grid_max)

        status, solver_used = self._solve_qp()
    ...
    def _solve_qp(self):
        ...
        for solver_name in self.solver_order:
            try:
                ...
                elif solver_name == "CLARABEL" and hasattr(cp, "CLARABEL"):
                    self._prob.solve(solver=cp.CLARABEL, warm_start=True, verbose=False)
                elif solver_name == "SCS":
                    self._prob.solve(solver=cp.SCS, warm_start=True, verbose=False, max_iters=10000, eps=1e-4)
                else:
                    continue
                last_status = str(self._prob.status)
                last_solver = solver_name
                if last_status == "optimal":
                    return last_status, last_solver
                if last_status == "optimal_inaccurate":
                    # Try a later solver for a cleaner certificate if available.
                    continue
            except Exception as ex:
                last_status = f"solve_error: {str(ex)[:80]}"
                last_solver = solver_name
                continue
        return last_status, last_solver
