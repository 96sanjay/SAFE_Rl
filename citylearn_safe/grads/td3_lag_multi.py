...
# ---------------------------------------------------------------------------
# Per-constraint cost keys (from safety_env info dict).
...
class TD3LagMulti(TD3):
    ...
    def attach_projector(self, projector: Any) -> None:
        """Attach DiffProjector and allocate proj_state buffer fields.
        ...
        if projector is not None:
            ...
            if dim > 0 and 'proj_state' not in self._buf.data:
                self._buf.data['proj_state'] = torch.zeros(
                    buf_shape, dtype=torch.float32, device=self._device,
                )
                print(f"[TD3LagMulti] Allocated proj_state buffer: dim={dim}")
            if dim > 0 and 'next_proj_state' not in self._buf.data:
                self._buf.data['next_proj_state'] = torch.zeros(
                    buf_shape, dtype=torch.float32, device=self._device,
                )
                print(f"[TD3LagMulti] Allocated next_proj_state buffer: dim={dim}")
