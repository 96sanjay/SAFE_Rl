"""SAC with per-constraint Lagrange multipliers (multi-lambda).

Extends OmniSafe's SAC with 5 independent Lagrange multipliers (one per
constraint) and per-constraint cost Q-critics. OmniSafe provides the RL
engine (SAC: replay buffer, reward critics, entropy tuning); this module
adds the multi-constraint CMDP layer on top.

Key design:
  - Inherits from SAC (not SACLag) — no orphaned single-cost machinery
  - 5 per-constraint cost Q-critics with independent PID/SGD lambdas
...
class _MultiCostOffPolicyAdapter(OffPolicyAdapter):
    ...
    def rollout(
        ...
        for _ in range(rollout_step):
            ...
            # Apply per-constraint cost weights (set by SACLagMulti._init)
            ...
            for i, key in enumerate(COST_KEYS):
                ...
                weighted_val = raw_val * cost_weights[i]
                per_cost_vals[f'cost_{i}'] = torch.tensor(
                    [weighted_val], dtype=torch.float32
                )
                # Raw cost → lambda PID updates (compared against cost_limits
                # in physical units, weights must not distort this)
...
@registry.register
class SACLagMulti(SAC):
    """SAC with per-constraint lambdas and softmax Q-cost weighting.
    ...
    Inherits from SAC (which inherits from DDPG -> BaseAlgo). OmniSafe
    provides: reward critics, entropy tuning, replay buffer, training loop.
    This class adds: per-constraint cost Q-critics, PID/SGD lambdas,
    softmax-weighted actor loss, cost weights, curriculum annealing.
    """
    ...
    def _init(self) -> None:
        ...
        # Weights multiply raw costs before storing in replay buffer.
        # This scales the Q-critic targets, making the actor loss more
        # sensitive to constraints with higher weights.
        # Lambda PID updates use RAW costs (physical units vs cost_limits).
        # Default 1.0 = raw costs (backward compatible with all configs).
        self._cost_weights = [
            float(getattr(multi_cfgs, f'cost_weight_{i}', 1.0))
            for i in range(NUM_COSTS)
        ]
        # Pass to adapter for use during rollout
        ...
        if any(w != 1.0 for w in self._cost_weights):
            print(f"[SACLagMulti] Cost weights: {self._cost_weights}")
    ...
    def _init_log(self) -> None:
        ...
        super()._init_log()
        # OmniSafe compatibility (was registered by SACLag parent)
    ...
    def _update_epoch(self) -> None:
        ...
        # Log max lambda for OmniSafe compatibility
        max_lam = max(self._get_lambda(i) for i in range(NUM_COSTS))
        self._logger.store({'Metrics/LagrangeMultiplier': max_lam})
    ...
    def _log_when_not_update(self) -> None:
        ...
        # OmniSafe compatibility (was logged by SACLag parent)
        max_lam = max(self._get_lambda(i) for i in range(NUM_COSTS))
        self._logger.store({'Metrics/LagrangeMultiplier': max_lam})
        for i in range(NUM_COSTS):
            self._logger.store({
