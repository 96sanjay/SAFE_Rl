#!/usr/bin/env python3
"""Train PPO-Lag with PID Lagrangian and BC warm-start for temperature case study.
...
from citylearn_safe.ppo_lag_temp_cooling_only import PPOLagTempCoolingOnly
...
def build_cfg(cfg_path: str) -> tuple[str, Config]:
    ...
    algo = custom.pop("algo", "PPOLagTempCoolingOnly")
...
def main(cfg_path: str) -> None:
    """Load config, run BC warm-start, then train PPOLag with PID."""
    ...
    cost_limit = float(getattr(pid_cfgs, "cost_limit", 55.0)) if pid_cfgs else 55.0

    print(f"\n{'=' * 60}")
    print("  PPOLagTempCoolingOnly -- PID Lagrangian + BC Warm-Start")
    print(f"  Env: {env_id}")
    ...
    agent = PPOLagTempCoolingOnly(env_id=env_id, cfgs=cfgs)
