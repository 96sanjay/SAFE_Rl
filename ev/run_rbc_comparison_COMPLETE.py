...
class IntelligentRBC:
    ...
    def __init__(self, env, ev_mode: str = "greedy"):
        ...
        # OLD (breaks when env is wrapped and has no action_names)
        # names = _unwrap_action_names(env.action_names)

        # NEW (robust): unwrap to raw CityLearn / base env to get action_names
        raw = unwrap_to_raw_citylearn_env(env)
        names = _unwrap_action_names(getattr(raw, "action_names", []))
        if not names:
            # fallback: try base env if present
    ...
    def _charger_connected_now(self, raw_env, action_name: str) -> bool:
        ...
        # OLD (buggy sometimes: off-by-one vs sim arrays)
        # t_state = int(getattr(raw_env, "time_step", 0))
        # if t_state < 0:
        #     t_state = 0

        # OLD (buggy sometimes: off-by-one vs action application)
        # t_state = int(current_time_index(raw_env))
        # if t_state < 0:
        #     t_state = 0

        # NEW (correct for your wrapper): action chosen now is stored at (time_step+1)
        # so we must check connection at the NEXT sim index.
